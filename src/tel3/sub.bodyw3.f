      SUBROUTINE BODYW13(iFlt,X,N,DT,IB,IC,F1,D1,A1,H,AZ,P,FC0,ZQP,ZQS,
     -                  ZI,dely,ND)
*===================================================*
...
*===================================================*
      PARAMETER (NL0=10,RADIUS=6371,PI=3.141593)
...
16      ll=l
	...
	vsrc=vp(ll)
* < Radiation pattern >
      CALL RADP3(iFlt,AZ-F1,D1,A1,P,PD,PU,SVD,SVU,SHD,SHU,VP(LL),VS(LL))
...
* P or PP wave
      IF(IB.EQ.1.OR.IB.EQ.4) THEN
        FC=FC0/(4*PI*sqrt(DEN(NL)*DEN1(1)*VP(NL)*VP1(1))*VP(NL)**2)
...
* SV-wave
      ELSEIF(IB.EQ.2) THEN
        FC=FC0/(4*PI*sqrt(DEN(NL)*DEN(1)*VS(NL)*VS(1))*VS(NL)**2)
...
* SH-wave
      ELSEIF(IB.EQ.3) THEN
        FC=FC0/(4*PI*sqrt(DEN(NL)*DEN1(1)*VS(NL)*VS1(1))*VS(NL)**2)
